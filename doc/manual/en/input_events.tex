...
\item[\texttt{BOX1--BOX29}] Virtual InfoBoxe key generated when the infoboxe number 1 to 29 is selected
...
Virtual InfoBoxe Key : 
BOX1--BOX29 are the Infobox function key name (like APP1) ,  generated when the infoboxe number 1 to 29 is selected
by the mouse during less than 1 seconds.
This is usefull for PDA without hardware key (only touch screen ) in order
to have virtuel KEY on screen.

The menu or event to be launch could be configure in  .xci file with the MODE=infobox : 
Without configuration in .xci file , the default behaviour is used. 

example: this will display the message "Infobox 1 pressed" 
when infobox 1 is pressed : 
...
type=key
data=BOX1
event=StatusMessage Infobox 1 pressed
